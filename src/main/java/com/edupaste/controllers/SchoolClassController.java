...
public class SchoolClassController {
    ...
    public ResponseEntity<?> getAll(
        ...
        Page<com.edupaste.payloads.SchoolClassResponse> pagedData = service.getAll(PageRequest.of(page - 1, limit));
        PagedResponse<com.edupaste.payloads.SchoolClassResponse> response = new PagedResponse<>(
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('SCHOOL_ADMIN')")
    public ResponseEntity<?> update(@PathVariable UUID id, @RequestBody SchoolClassRequest request) {
        Map<String, Object> response = new HashMap<>();
        response.put("success", true);
