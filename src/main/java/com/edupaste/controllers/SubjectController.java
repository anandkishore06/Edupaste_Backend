...
public class SubjectController {
    ...
    public ResponseEntity<?> getAll(
        ...
        Page<com.edupaste.payloads.SubjectResponse> pagedData = service.getAll(PageRequest.of(page - 1, limit));
        PagedResponse<com.edupaste.payloads.SubjectResponse> response = new PagedResponse<>(
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('SCHOOL_ADMIN')")
    public ResponseEntity<?> update(@PathVariable UUID id, @RequestBody SubjectRequest request) {
        Map<String, Object> response = new HashMap<>();
        response.put("success", true);
