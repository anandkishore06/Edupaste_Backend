...
public class SectionController {
    ...
    public ResponseEntity<?> getAll(
        ...
        Page<com.edupaste.payloads.SectionResponse> pagedData = service.getAll(PageRequest.of(page - 1, limit));
        PagedResponse<com.edupaste.payloads.SectionResponse> response = new PagedResponse<>(
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('SCHOOL_ADMIN')")
    public ResponseEntity<?> update(@PathVariable UUID id, @RequestBody SectionRequest request) {
        Map<String, Object> response = new HashMap<>();
        response.put("success", true);
